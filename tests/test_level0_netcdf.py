...
def test_build_level0_netcdf_writes_scc_acquisition_metadata(tmp_path: Path) -> None:
    ...
    build_level0_netcdf(
        netcdf_path=str(output_path), save_id="20240101sant", period="nt",
        lidar_data=_lidar_data(), group_df=_group_df(tmp_path, include_dark_current=False),
        weather_data={"temperature_c": 23.0, "pressure_hpa": 935.0}, config=_config(), logger=logging.getLogger("test"),
    )
    with xr.open_dataset(output_path) as ds:
        ...
        assert np.isnan(float(ds["DAQ_Range"].isel(channels=1).values))
...
def test_build_level0_netcdf_truncates_time_axis_and_shots(tmp_path: Path) -> None:
    ...
    build_level0_netcdf(
        netcdf_path=str(output_path), save_id="20240101sant", period="nt", lidar_data=lidar_data,
        group_df=_group_df(tmp_path, include_dark_current=False), weather_data={"temperature_c": 23.0, "pressure_hpa": 935.0},
        config=_config(), logger=logging.getLogger("test"),
    )
    with xr.open_dataset(output_path) as ds:
...
def test_build_level0_netcdf_uses_fallback_channel_id_when_missing_from_config(tmp_path: Path) -> None:
    ...
    build_level0_netcdf(
        netcdf_path=str(output_path), save_id="20240101sant", period="nt", lidar_data=_lidar_data(),
        group_df=_group_df(tmp_path, include_dark_current=False), weather_data={"temperature_c": 23.0, "pressure_hpa": 935.0},
...
def test_build_level0_netcdf_accepts_flat_shared_channel_id_mapping(tmp_path: Path) -> None:
    ...
    build_level0_netcdf(
        netcdf_path=str(output_path), save_id="20240101sant", period="pm", lidar_data=_lidar_data(),
        group_df=_group_df(tmp_path, include_dark_current=False), weather_data={"temperature_c": 23.0, "pressure_hpa": 935.0},
        config=config, logger=logging.getLogger("test"),
    )
    with xr.open_dataset(output_path) as ds:
...
def test_build_level0_netcdf_writes_dark_current_scc_times_and_provenance(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_parse_licel_group(files: list[str], logger: logging.Logger) -> dict:
        return {"channels": ["532.AN", "532.PC"], "tensors": {"532.AN": np.ones((1, 4)) * 0.1, "532.PC": np.ones((1, 4)) * 0.2}}
    monkeypatch.setattr(netcdf_module, "parse_licel_group", fake_parse_licel_group)
    output_path = tmp_path / "level0.nc"
    build_level0_netcdf(
        netcdf_path=str(output_path), save_id="20240101sant", period="nt", lidar_data=_lidar_data(),
        group_df=_group_df(tmp_path, include_dark_current=True), weather_data={"temperature_c": 23.0, "pressure_hpa": 935.0},
        config=_config(), logger=logging.getLogger("test"),
    )
    with xr.open_dataset(output_path) as ds:
        validate_level0_contract(ds)
        assert "Background_Profile" in ds
        assert "Raw_Bck_Start_Time" in ds and "Raw_Bck_Stop_Time" in ds
...
def test_build_level0_netcdf_flags_missing_dark_current_channel(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import milgrau.level0.netcdf as netcdf_module
    def fake_parse_licel_group(files: list[str], logger: logging.Logger) -> dict:
        return {"channels": ["532.AN"], "tensors": {"532.AN": np.ones((1, 4)) * 0.1}}
    monkeypatch.setattr(netcdf_module, "parse_licel_group", fake_parse_licel_group)
