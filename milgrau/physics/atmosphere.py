"""US Standard Atmosphere 1976 thermodynamic fallback.

This module is pure physics: deterministic, network-free and independent of
pipeline level. External atmospheric profiles are acquired by :mod:`milgrau.io`;
Level 1 materializes the selected atmosphere on the lidar grid.
"""
...
def get_standard_atmosphere(altitude_array_m: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    seven USSA76 layers through 84.852 km geopotential altitude. Values above
    that formal limit are rejected instead of being silently extrapolated.
    """
