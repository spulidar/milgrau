...
def _hardware_map(config: Mapping[str, Any], period: str) -> Mapping[str, Any]:
    ...
    - ``hardware.name_to_id.day/night`` for legacy period-specific IDs
    - ``hardware.name_to_id.<channel_name>`` for one SCC-stable shared mapping
    """
    name_to_id = config.get("hardware", {}).get("name_to_id", {})
    if not isinstance(name_to_id, Mapping):
        return {}

    # Prefer a flat shared mapping when channel names are declared directly.
